...
export const buttonClassName = ({ bgColor , isValid , isOutline}) => {
     if(isOutline){
          ...
          return `focus:ring-2 shadow-${bgColor}-400  ring-offset-2 shadow-sm md:shadow-md  lg:shadow-lg ${isValid ? `ring-${bgColor}-400 hover:bg-${bgColor}-700 bg-${bgColor}-600 border   text-${bgColor}-100 cursor-pointer ` : "ring-gray-400 cursor-not-allowed hover:bg-gray-800 bg-gray-700 border border-gray-600 text-gray-100"}  py-2 px-6 font-sans  text-sm rounded-md`
     }
}
...
export const linkClassName =  ({ bgColor , isOutline}) => {
     if(isOutline) {
          ...
          return `focus:ring-2 shadow-${bgColor}-400 ring-offset-2 shadow-sm md:shadow-md  lg:shadow-lg  ring-${bgColor}-600 hover:bg-${bgColor}-700 bg-${bgColor}-600 border   text-${bgColor}-100 cursor-pointer  py-2 px-6 font-sans  text-sm rounded-md`
     }
}
