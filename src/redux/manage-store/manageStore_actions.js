...
const adminToken = "dhCtSsYhqf1nqZ6GOeHi9A29pHI0iVhp2ITHyBkoYxTvrBCkWuFdd3qXvVf5"
...
export const fetchBaseProducts = ({ page, limit, paramsBrand,barcode, paramsCategory, name}) => dispatch => {
    ...
    axios.get(encodeURI(`https://market-api.iran.liara.run/api/admin/products?state=active&order=desc&title=${name || ""}&barcode=${barcode || ""}&category_id=${paramsCategory || ""}&brand_id=${paramsBrand ||""}&page=${page || 1}&limit=${limit || 12}`) , {headers : {authorization : `Bearer ${adminToken}`}})
...
export const fetchCategories = () => dispatch => {
    ...
    axios.get(`https://market-api.iran.liara.run/api/admin/categories?list=1` , {headers : {authorization : `Bearer ${adminToken}`}})
...
export const fetchBrands = () => dispatch => {
    ...
    axios.get(`https://market-api.iran.liara.run/api/admin/brands?list=1` , {headers : {authorization : `Bearer ${adminToken}`}})
