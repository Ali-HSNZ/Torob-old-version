...
export const fetchCartItems = () => dispatch => {
     ...
     .then(({data}) => dispatch(addToCartSuccess(data)))
     .catch(error => {
          requestError({error : error?.response?.data?.errors , defaultMessage : "خطای سرور در بخش گرفتن لیست سبد خرید"})
          dispatch(addToCartFailure("خطای سرور در بخش گرفتن لیست سبد خرید "))
     })
}