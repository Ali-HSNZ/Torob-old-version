...
const SelectBoxForCategories = ({placeholder , selectedCategory , categoryQuery , setSelectedCategory , filteredCategories , setCategoryQuery}) => {
    return ( 
        <div className="flex">
            <Combobox value={selectedCategory} onChange={setSelectedCategory}>
                <div className="relative w-full">
                    <div className="relative  focus:outline-none ">
                        <Combobox.Input  placeholder={placeholder || ""}  onChange={(event) => setCategoryQuery(event.target.value)}  className=" w-full border-gray-300 hover:border-gray-600 pl-12 focus:border-gray-600 focus:ring-0 text-sm font-sans bg-white text-gray-800 rounded-md" displayValue={(brand) => brand.name}/>
                        ...
                        <button type={"button"} onClick={()=>setSelectedCategory("") } className="absolute group inset-y-0 top-[0px] left-7 flex  items-center ">
...
export default SelectBoxForCategories;