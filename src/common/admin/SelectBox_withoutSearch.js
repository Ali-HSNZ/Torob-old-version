    ...
    <Listbox value={selected} onChange={setSelected} >
      <div className="relative mt-1">
        <Listbox.Button className="relative w-full rounded-md bg-white py-2 border border-gray-300 pr-4 text-right hover:border-gray-500 sm:text-sm cursor-pointer">
          <span  className="block truncate font-iranyekan-regular text-gray-800">{selected.name || selected}</span>
          ...
          <Listbox.Options className="absolute mt-1 z-50 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            {data.map((data, dataIdx) => (
              <Listbox.Option key={dataIdx} className={({ active }) => `relative cursor-pointer select-none py-2 pl-10 pr-4 ${active ? 'bg-gray-100 text-gray-900' : 'text-gray-900'}`} value={data}>
                {() => <span className={`block truncate font-iranyekan-regular text-gray-800`}>{data.name}</span>}
              </Listbox.Option>
            ))}
          </Listbox.Options>
        </Transition>
