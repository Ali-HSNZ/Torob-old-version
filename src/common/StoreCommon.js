          ...
          <section>
                    ...
                    <section className="flex flex-col sm:flex-row w-full justify-between items-start sm:items-center">
                         <div className="flex ">
                              ...
                              <div className='mr-2 whitespace-nowrap flex w-fit gap-y-1 flex-row  items-center xl:items-start '>
                                   <p className='font-iranyekan-regular font-iranyekan-bold text-gray-800'>{store.title}</p>
                                   <p className='font-iranyekan-regular text-xs text-gray-500 mr-1'>({store.province})</p>
                              </div>
                         </div>
                         {store.discounts.length > 0 && <label onClick={button => rotateChevron(button.currentTarget)} htmlFor={`discount_${store.store_id}`} className="select-none py-2 px-4 bg-gray-50 hover:border-red-600 group-hover:bg-white rounded-lg flex items-center  border cursor-pointer">
                              ...
                              <span className="whitespace-nowrap font-iranyekan-regular text-sm mr-1 text-gray-700">تخفیف پله‌ایی</span>
                         </label>}
                    </section>
                    <section className="flex flex-col-reverse  sm:flex-row items-center gap-x-5 justify-between    sm:mr-4">
                         <p className={`text-red-600   font-iranyekan-bold whitespace-nowrap font-iranyekan-regular `}>{toPersianPrice(store.price) +" تومان " }</p>
                         {availableProductInCart() ? (
                              <div className="flex bg-white  border-2 border-red-500 rounded-md items-center mb-4 sm:m-0 overflow-hidden">
                                   {/* increase */}
                                   <button disabled={limitHandler()} onClick={()=>dispatch(increaseProductToCart({product_id : store.product_id , store_id : store.store_id}))} className="font-iranyekan-regular text-sm p-2 disabled:cursor-not-allowed  text-gray-800 disabled:bg-red-100">
                                   ...
                                   {isIncreaseProductLoading({store_id : store.store_id}) ? (
                                        ...
                                        <span className="font-iranyekan-regular text-sm px-3">{toPersianDigits(availableProductInCart().count)}</span>
                                   )}
                                   {/* Decrease */}
                                   <button onClick={()=>dispatch(decreaseProductToCart({product_id : store.product_id , store_id : store.store_id}))} className="font-iranyekan-regular text-sm p-2">
               ...
               <section className="peer-checked:inline-block  hidden w-full px-4 ">
                    {store.discounts.length > 0 && <div className="w-full mt-2 border border-gray-200 overflow-hidden rounded-lg pt-2 mb-4">
                         ...
                         <p className="font-iranyekan-regular text-sm font-iranyekan-bold">نوع</p>
                         <p className="font-iranyekan-regular text-sm font-iranyekan-bold">مقدار </p>
                         <p className="font-iranyekan-regular text-sm font-iranyekan-bold">قیمت نهایی</p>
                         </div>
                         {store.discounts.map((discount,index) => (
                              <div key={index} className="px-4 w-full grid grid-cols-3 self-center py-2 odd:bg-gray-200 even:bg-gray-100">
                                   <p className="font-iranyekan-regular text-sm px-1 flex items-center sm:p-0">{discount.discount_type === 'count' ? "تعداد" : "قیمت"}</p>
                                   <p className="font-iranyekan-regular text-sm px-1 sm:p-0 flex items-center">{toPersianPrice(discount.discount_value)} {discount.discount_type === 'price' ? "تومان" : ''} </p>
                                   
                                   <div>
                                        {discount.discount_type == 'count' && <p className="flex items-center font-iranyekan-regular text-sm px-1 sm:p-0 line-through text-red-600">{toPersianPrice(store.price * discount.discount_value)}</p>}
                                        <p className="font-iranyekan-regular text-sm px-1 flex items-center sm:p-0">{toPersianPrice(discount.final_price)} تومان</p>
                                   </div>
                              </div>
                         ))}
                    </div>}
