    ...
    return (  
            ...
            {/* //? Input Search =>  */}
                    ...
                    <form onSubmit={(e)=> {e.preventDefault() ; router.push({pathname : "/search" , query : {query : inputValue}})}} method='get' className="w-full hidden pr-6 md:flex md:justify-center lg:justify-start items-center">
                            <input className="bg-white text-gray-800 w-1/2 py-3 sm:w-9/12 font-sans border lg:w-[420px] border-gray-300 px-4" value={inputValue} onChange={input => setInputValue(input.target.value)} placeholder="نام کالا را وارد کنید"/>
            ...
            {/* //? Mobile Search Input For Mediom With =>  */}
                            ...
                            data&&data.map((category,index) => {
                                return(
                                    <nav className="flex gap-x-4 rela" key={index}>
                                        <button  className="hover:text-red-500  cursor-pointer font-sans text-gray-800" onClick={()=> handleCategory(category.id) & setIsModal(false)}>{category.name}</button>
                                            ...
                                            <div className="bg-gray-50 pb-4">
                                                <nav className="px-5 py-4">
                                                    <Link href={{pathname : '/search' , query : {category:category.name}}}>
                                                        <a onClick={()=> closeCategory()} className="hover:text-red-500 font-bold font-sans text-gray-800">{category.name}</a>
                                                    </Link>
                                                </nav>
                                                <hr/>
                                                        ...
                                                        return(
                                                            <div className="mr-10 mt-4" key={index}>
                                                                ...
                                                                <Link href={{pathname : '/search' , query : {category:sub.name}}}>
                                                                    <a onClick={()=> closeCategory()} className="text-gray-800 hover:text-red-500 font-sans font-bold cursor-pointer " >{sub.name}</a>
                                                                </Link>
                                                                </nav>
                                                                {sub.sub_categories && sub.sub_categories.length > 0 && sub.sub_categories.map((sub_sub,index) => {
                                                                    return(
                                                                        <nav className="flex gap-x-4 mr-4 mt-2 " key={index}>
                                                                            <Link href={{pathname : '/search' , query : {category:sub_sub.name}}}>
                                                                            <a onClick={()=> closeCategory()} className="hover:text-red-500 font-sans cursor-pointer text-gray-800" >{sub_sub.name}</a>
                                                                            </Link>
                                                                        </nav>
                                                                    )
                                                                })}
                ...
                <section className={`w-full sm:w-2/5  fixed  ${isOpen ? "" : "hidden"} inset-0  z-40 md:hidden w-full bg-white`}>
                    ...
                    <div className="px-4 mt-6 flex w-full">
                        ...
                        <h6 className="text-sm  text-center w-full font-sans text-gray-800">همه‌ دسته‌بندی ها</h6>
                    </div>
                    ...
                    <div className="flex py-4 gap-x-4 mt-4 z-20 w-full whitespace-nowrap overflow-x-auto px-3">                    
                        {data&&data.map((category,index) => {
                            return(
                                <button key={index}  className="hover:text-red-500 text-sm border px-4 py-2 rounded-md border-gray-400 cursor-pointer flex font-sans text-gray-800" onClick={()=> handleCategory(category.id)}>{category.name}</button>
                            )
                        })}
                    </div>
                    {
                                ...
                                <div className={`${category.status ? "" : "hidden"}  z-30 absolute px-5 right-0 left-0 rounded-md`}>
                                    ...
                                    <div>
                                        <section className="py-4">
                                            <button onClick={()=>setCurrentCategory(category.name)} className={`hover:text-red-500 text-gray-800 ${currentCategory === category.name && "text-red-500"} font-bold font-sans`}>{category.name}</button>
                                        </section>
                                        <hr/>
                                                    ...
                                                    return(
                                                        <div className="mt-4 overflow-x-auto pb-3" key={index}>
                                                            <section className="flex gap-x-4 ">
                                                                <button  onClick={()=>setCurrentCategory(sub.name)} className={`hover:text-red-500 ${currentCategory === sub.name && "text-red-500"} font-sans font-bold cursor-pointer text-gray-800`}>{sub.name}</button>
                                                            </section>
                                                            {sub.sub_categories && sub.sub_categories.length > 0 && sub.sub_categories.map((sub_sub,index) => {
                                                                return(
                                                                <section className="flex gap-x-4 mt-3 text-gray-600  whitespace-nowrap" key={index}>
                                                                    <button  onClick={()=>setCurrentCategory(sub_sub.name)} className={`hover:text-red-500 ${currentCategory === sub_sub.name && "text-red-500"} font-sans cursor-pointer text-gray-800`} >{sub_sub.name}</button>
                                                                </section>
                                                                )
                                                            })}
                                                        </div>
                    ...
                    <div className="bg-gray-100 flex gap-x-4 w-full h-auto absolute bottom-0 py-4 px-4">
                        ...
                        <button onClick={()=> {delete query.priceMin & delete query.priceMax &  router.push({pathname : "/search" , query : {...query}}) & closeCategory() & setIsOpen(false)}}  className="w-1/4 border border-gray-700 rounded-md text-sm font-sans text-gray-800  py-3">حذف</button>
