...
export  async function getServerSideProps(ctx){
    ...
    const {query : productName , fromPrice , toPrice , category , available , sort , brand } = ctx.query;
    ...
    const mainSearch =  await axios.get(encodeURI(`https://project-torob-clone.iran.liara.run/api/search?${productName?"&q="+productName:""}&perPage=10&page=1${brand ? "&brand="+brand : ""}${available ? "&available="+brand : ""}${category ? "&category="+category : ""}${sort ? `&sort=${sort}` : "" }${fromPrice ? "&fromPrice="+fromPrice : ""}${toPrice ? "&toPrice="+toPrice : ""}`) , {headers : {Authorization : `Bearer ${token}`} })
