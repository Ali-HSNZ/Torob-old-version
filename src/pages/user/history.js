            ...
            <div className="flex justify-between mt-5">
              ...
              {history && history.length > 0 && (
                <button
                  ...
                  className="font-sans  text-sm px-3 hover:text-red-600 rounded-lg py-2 hover:bg-red-100 border border-gray-100 hover:border-red-200 "
                >
                  حذف مشاهدات اخیر
                </button>
              )}
              ...
              <section className={Styles.productsParent}>
                {history.map((product, index) => {
                  const isLiked = () => {
                    const likedProduct =
                      likes &&
                      likes.find((item) => item.hash_id === product.hash_id);
                    if (likedProduct) {
                      return true;
                    }
                    return false;
