...
import empty_watched from '/src/images/empty_watched.png'
...
const Analytics = () => {
    ...
    const renderAnalytics = analytics && analytics.slice(analyticsShown , analyticsShown + numOfAnalyticsOnPage).map((product,index) => {
        ...
        return <AnalyticsComponent key={index} user={user && user.phone_number ? true : false} isAnalyze={isAnalyze()} isAnalyzeLoading={isAnalyzeLoading()} isLikeLoading={isLikeLoading()} isLiked={isLiked()} product={product}/>
    })
