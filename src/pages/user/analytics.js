...
const Analytics = () => {
     ...
     const pageCount = analytics ? Math.ceil(analytics.length / numOfAnalyticsOnPage) : 1;
     const paginationHandler = (event, value) => {
          setAnalyticsPage(value - 1);
          window.scroll({ top: 0 });
     };
     // const renderAnalytics = analytics && analytics .slice(analyticsShown, analyticsShown + numOfAnalyticsOnPage).map((product, index) => {
     //      const isLiked = () => {
     ...
     //           if(loadingProduct) return true; else return false
     //      };
     //           return <AnalyticsComponent key={index} user={user && user.phone_number_primary ? true : false} isAnalyze={isAnalyze()} isAnalyzeLoading={isAnalyzeLoading()} isLikeLoading={isLikeLoading()} isLiked={isLiked()} product={product} />
     // });
                    ...
                    <section className="w-full lg:w-4/5 flex-0 h-max px-4 ">
                         ...
                         <div className="flex justify-between items-center mt-4">
                              <div className="flex items-center">
                                   ...
                                   <span className="font-sans text-gray-800 font-bold text-xl"> تغییرات قیمت</span>
                              </div>
                         </div>
                         )}
                         {/* {!loading && !analytics && ( */}
                              <div className="w-full flex justify-center my-7 lg:mt-7">
                                   <div className="w-full sm:w-1/2 flex flex-col items-center">
                                        <img alt="تصویر چارت " className="w-full h-auto" src={empty_watched.src} />
                                        <p className=" text-center leading-5 font-sans relative text-gray-500 sm:left-0 sm:w-4/6 text-xs">اعلان قیمت را برای محصولات دلخواه خود فعال کنید تا از موجودی و تغییرات قیمت‌شان مطلع شوید.</p>
                                   </div>
                              </div>
                         {/* )} */}
                         {/* {loading && (
                         ...
                         {analytics && (
                              <>
                                   <section className="h-max flex flex-col gap-y-4  mt-5"> {renderAnalytics} </section>
                                   <div className="w-full flex justify-center my-7">
                                        <Pagination dir="ltr" count={pageCount} color="primary" onChange={paginationHandler} />
                                   </div>
                              </>
                         )} */}
                    </section>
