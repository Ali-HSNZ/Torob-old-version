    ...
    return (  
            <div className={`w-full flex ${category ? " justify-between" : "justify-end"}   z-10 font-sans relative items-center`}>
                ...
                <div className="max-w-fit py-2  cursor-pointer hidden  md:flex flex-col justify-end  items-end">
                    ...
                    <Button sx={{fontFamily : 'iranyekan' , fontSize : "14px", color : '#545353'}} id="basic-button" aria-controls={open ? 'basic-menu' : undefined} aria-haspopup="true" aria-expanded={open ? 'true' : undefined} onClick={handleClick} className="rounded-md">
                        {currentFilter}
                    ...
                    <Menu anchorEl={anchorEl} open={open} onClose={handleClose}>
                        <div className="flex flex-col">
                            <section  onClick={() => router.push({pathname : '/search' , query : {...query , sort : "mostFavorite"}}) &  handleClose() & setCurrentFilter("محبوب‌ ترین")}>
                                <p className={`${query.sort && query.sort === "mostFavorite" ? "bg-gray-200" : ""} py-1.5 px-4 w-full font-sans cursor-pointer text-sm text-center hover:bg-gray-100`}>محبوب‌ ترین</p>
                            </section>

                            <section onClick={() => router.push({pathname : '/search' , query : {...query , sort : "dateRecent"}}) & handleClose() & setCurrentFilter("جدید ترین")}>
                                <p className={`${query.sort && query.sort === "dateRecent" ? "bg-gray-200" : ""} py-1.5 px-4 w-full font-sans cursor-pointer text-sm text-center hover:bg-gray-100`}>جدید ترین</p>
                            </section>

                            <section onClick={() =>  router.push({pathname : '/search' , query : {...query , sort : "priceMin"}}) & handleClose() & setCurrentFilter("ارزان ترین")}>
                                <p className={`${query.sort && query.sort === "priceMin" ? "bg-gray-200" : ""} py-1.5 px-4 w-full font-sans cursor-pointer text-sm text-center hover:bg-gray-100`}>ارزان ترین</p>
                            </section>

                            <section onClick={() => router.push({pathname : '/search' , query : {...query , sort : "priceMax"}}) & handleClose() & setCurrentFilter("گران ترین")}>
                                <p className={`${query.sort && query.sort === "priceMax" ? "bg-gray-200" : ""} py-1.5 px-4 w-full font-sans cursor-pointer text-sm text-center hover:bg-gray-100`}>گران ترین</p>
                            </section>
                        </div>
