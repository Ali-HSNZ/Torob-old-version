        ...
        {panelType === "normal" ? (
               ...
               <section className="mt-4 ">
                    ...
                    <div className="w-full flex justify-center relative items-center mt-6 ">
                         <button type={"submit"} onClick={()=> dispatch(authPanel({isOpen : false , type : 'normal'})) & router.push({pathname:formik.values.signupStatus === 'user' ? "/signup/user" : "/signup/store"})}  className="duration-150 text-center w-10/12 py-1.5 disabled:bg-gray-700 disabled:hover:bg-gray-800 disabled:cursor-not-allowed text-gray-50 rounded-md font-sans bg-red-600 hover:bg-red-700  shadow-md text-sm">
                    ...
                    <button type={"button"} onClick={()=> dispatch(changePanelType('userPass'))} className="text-xs text-center pb-4 w-full font-sans mt-4 text-blue-700 hover:underline hover:underline-offset-4 outline-none">ورود به حساب کاربری</button>
               </section>
               </form>
        ) : (
          <form onSubmit={formik.handleSubmit} className="bg-[#ffffff] rounded-md p-4 w-full sm:w-[400px] outline-none" >
               ...
               <hr className="mt-8 border border-gray-200" />
               <div className="mt-[-15px] font-sans w-full flex justify-center">
                    <span className="bg-white px-3 text-gray-600 text-sm"> ورود به حساب کاربری</span>
               </div>
               <section className="mt-4">
