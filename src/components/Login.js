...
const Login = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: user
      ? withOutUserVerifyCode_Validation
      : withUserVerifyCode_Validation,
  ...
  return (
    <Modal
      open={panel || false}
      onClose={() => dispatch(authPanel(false))}
      className="flex justify-center items-center px-4"
    >
      <form
        onSubmit={formik.handleSubmit}
        className="bg-[#ffffff] rounded-md p-4"
      >
        <a
          onClick={() => dispatch(authPanel(false))}
          className="flex w-fit justify-end float-left cursor-pointer"
        >
          <svg
        ...
        <section className="mt-4 px-10">
          <div>
            ...
            <input
              ...
              className={` w-full mt-3 font-sans  py-2 bg-gray-50 text-sm p-4 text-gray-800 rounded-md border border-gray-300 outline-none`}
            />
          </div>
          {formik.errors["phone_number"] &&
            formik.touched["phone_number"] && (
              <span className={" pt-2 w-full font-sans text-sm text-red-500"}>
                {formik.errors["phone_number"]}
              </span>
            )}
          ...
          <div className="w-full flex justify-center items-center mt-6 ">
            <button
              ...
              className="w-10/12 py-1.5 disabled:bg-gray-700 disabled:cursor-not-allowed text-gray-200 rounded-md font-sans bg-red-700 text-sm"
            >
              {user
                ? `ورود به حساب کاربری${loading ? "..." : ""}`
                : `دریافت کد تایید${loading ? "..." : ""}`}
            </button>
          </div>
          ...
          <p className="text-xs text-center pb-4 w-full font-sans mt-4 text-blue-700">
