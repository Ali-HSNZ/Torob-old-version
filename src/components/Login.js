        ...
        {panelType === "normal" ? (
            <form  className="bg-[#ffffff] rounded-md p-4 w-full sm:w-[400px] outline-none" >
              ...
              <section className="mt-4 ">
                    <div>
                      ...
                      <label htmlFor="user" className={`cursor-pointer flex items-start rounded-xl bg-white p-4 border-2 peer-checked:border-red-200 peer-checked:shadow-md peer-checked:bg-red-50 border-white hover:bg-red-50 `}>
                  ...
                  <div className="mt-4">
                    ...
                    <label htmlFor="store" className={`cursor-pointer flex items-start rounded-xl bg-white p-4 border-2 peer-checked:border-red-200 peer-checked:shadow-md peer-checked:bg-red-50 border-white hover:bg-red-50 `}>
                        <div className="flex h-12 w-12 items-center justify-center rounded-full  border-red-100 bg-red-50">
                ...
                <div className="w-full flex justify-center relative items-center mt-6 ">
                    <button type={"submit"} onClick={()=> dispatch(authPanel({isOpen : false , type : 'normal'})) & router.push({pathname:formik.values.signupStatus === 'user' ? "/signup/user" : "/signup/store"})}  className="text-center w-10/12 py-1.5 disabled:bg-gray-700 disabled:hover:bg-gray-800 disabled:cursor-not-allowed text-gray-50 rounded-md font-sans bg-red-600 hover:bg-red-700  shadow-md text-sm">
                        {formik.values.signupStatus === 'store' ? "ثبت نام فروشگاه" : "ثبت نام کاربر"}
                    </button>
                    {loading && <ReactLoading  className="absolute top-1 left-[-13px]" type="spinningBubbles" height={25} width={25} color="red" />}
                </div>
                <button type={"button"} onClick={()=> dispatch(changePanelType('userPass'))} className="text-xs text-center pb-4 w-full font-sans mt-4 text-blue-700 hover:underline hover:underline-offset-4 outline-none">ورود با نام کاربری و رمز عبور</button>
              </section>
            </form>
        ) : (
          <form onSubmit={formik.handleSubmit} className="bg-[#ffffff] rounded-md p-4 w-full sm:w-[400px] outline-none" >
              ...
              <section className="mt-4">
                ...
                <button type={"button"} onClick={()=> dispatch(changePanelType('normal'))} className="outline-none text-xs text-center pb-4 w-full font-sans mt-4 text-blue-700 hover:underline hover:underline-offset-4">ثبت نام به عنوان کاربر / فروشگاه</button>
              </section>
          </form>
