...
const Properties = ({product}) => {
    return (  
        <div className={`${Styles.properties}  bg-white max-h-[1200px] px-5 py-5`}>
            <p className='font-sans font-bold pb-3 text-right text-[18px] text-gray-800'>مشخصات {product.title}</p>
            
            <div className={`${Styles.properties_scroll}  overflow-y-auto max-h-[1200px]`}>
                <div className={` mt-4 w-full text-right`}>
                    <p className=' font-sans font-bold text-gray-800'>مشخصات کلی</p>
                    <hr className='mt-2'/>
                                ...
                                {generalInformation.map((properties,newIndex) => {
                                    return(
                                        <div className='my-5' key={newIndex}>
                                            <p className='font-bold font-sans text-sm text-gray-800'>{properties.name}</p>
                ...
                <div className='mt-4 w-full text-right'>
                    <p className=' font-sans font-bold text-gray-800 '>مشخصات فنی </p>
                    <hr className='mt-2'/>
                    {JSON.parse(product.specs).map((e,index) => {
